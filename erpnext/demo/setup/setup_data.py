...
def setup_user_roles(domain):
	...
	if not frappe.db.get_global('demo_hr_user'):
		user = frappe.get_doc('User', 'CaitlinSnow@gmail.com')
		...
		for d in frappe.get_all('User Permission', filters={"user": "CaitlinSnow@gmail.com"}):
			frappe.delete_doc('User Permission', d.name)

	if not frappe.db.get_global('demo_sales_user_1'):
		user = frappe.get_doc('User', 'VandalSavage@gmail.com')
	...
	if not frappe.db.get_global('demo_sales_user_2'):
		user = frappe.get_doc('User', 'GraceChoi@gmail.com')
	...
	if not frappe.db.get_global('demo_purchase_user'):
		user = frappe.get_doc('User', 'MaxwellLord@gmail.com')
	...
	if not frappe.db.get_global('demo_manufacturing_user'):
		user = frappe.get_doc('User', 'NeptuniaAquaria@gmail.com')
	...
	if not frappe.db.get_global('demo_stock_user'):
		user = frappe.get_doc('User', 'HollyGranger@gmail.com')
	...
	if not frappe.db.get_global('demo_accounts_user'):
		user = frappe.get_doc('User', 'BarryAllen@gmail.com')
	...
	if not frappe.db.get_global('demo_projects_user'):
		user = frappe.get_doc('User', 'PeterParker@gmail.com')
	...
	if domain == "Education":
		if not frappe.db.get_global('demo_education_user'):
			user = frappe.get_doc('User', 'ArthurCurry@gmail.com')
			user.add_roles('Academics User')
			update_employee_department(user.name, 'Management')
			frappe.db.set_global('demo_education_user', user.name)

	#Add Expense Approver
	user = frappe.get_doc('User', 'ClarkKent@gmail.com')
